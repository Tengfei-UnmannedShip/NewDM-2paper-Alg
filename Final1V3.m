...
for  OS=1:1:Boat_Num
    ...
    x1=WayPoint_temp1(1,1);
    y1=WayPoint_temp1(1,2);
    x2=WayPoint_temp1(2,1);
    y2=WayPoint_temp1(2,2);
    x3=WayPoint_temp1(3,1);
    y3=WayPoint_temp1(3,2);
    ...
    [WayPoint_OS,allpoint,R] =ScenarioWaypoint( Risk_value1(OS,:)',WayPoint_temp1);
    Boat(OS).waypoint=[Boat(OS).waypoint;WayPoint_OS];
    Boat(OS).allwaypoint=[Boat(OS).allwaypoint;allpoint];
    Boat(OS).Rwp=[Boat(OS).Rwp;R];  %为了防止结果为虚数，最终的R值
    ...
    [WayPoint_OS,allpoint,R]=ScenarioWaypoint( Risk_value2(OS,:)',WayPoint_temp1);
    Boat(OS).waypoint=[Boat(OS).waypoint;WayPoint_OS];
    Boat(OS).allwaypoint=[Boat(OS).allwaypoint;allpoint];
    Boat(OS).Rwp=[Boat(OS).Rwp;R];
    % 3.失控：直接向着目标点
                ...
                for TS=1:1:Boat_Num
                    if TS~=OS
                        ...
                        if  dis_now<Dis0
                            ...
                            Boat(OS).decision_label=Boat(OS).decision_label+0;
                        end
                    end
                end
                RiskMap=1./(ScenarioMap+1);
                
                %% 目标点确定，再不更新目标点的情况下，decision_label不变
                %    判断方法：如果已经到达场景路径点，则改为最终的目标点
                DisWP_temp=Boat(OS).pos-Boat(OS).currentWP;
                DisWP=norm(DisWP_temp);
                if  DisWP<4*Res   %已到达路径点，目标点为终点
