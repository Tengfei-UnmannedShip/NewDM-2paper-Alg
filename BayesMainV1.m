                    ...
                    if  Boat(OS).infer_label==0   %之前没有推测过，这是第一次
                        for  TS=1:1:Boat_Num
                            ...
                            if TS~=OS && CurrentRisk==1
                                ...
                                PrTheta_his0=[t,TS,TS,1];
                                ThetaList0=[t,TS,TS,Boat(TS).goal,PrTheta_his0(1,4)];
                                ...
                                for ts_infer=1:1:Boat_Num
                                    ...
                                    if ts_infer~=TS && CurrentRisk_TS==1 %TS视角下，ts_infer(包括本船OS)和TS的确有碰撞风险
                                        ...
                                        if  Boat(OS).infer_label==0   %之前没有推测过，这是第一次
                                            ...
                                            PrTheta_his0=[PrTheta_his0;t,TS,ts_infer,Pr1;t,TS,ts_infer,Pr2];
                                            ...
                                            %TODO 思考怎么把Pr1,Pr2从上一个时刻顺接到当前时刻                                           
                                        end
                                ...
                                [RR_points,PrX_eq1] = BayesEqu1( Boat_x,Boat_y,Boat_theta,Boat(TS).speed,Theta,MapSize,Res);
                                %% 步骤2.公式(2)更新当前的意图分布
                                % 来自树武程序
                                PrTheta0=PrTheta_his0(:,4)/sum(PrTheta_his0(:,4)); %PrTheta归一化，防止出现越来越小的情况
                                ...
                                PrTheta = PrTheta/sum(PrTheta(:));  %公式（2）
                                %% 步骤3.根据意图概率生成每一个路径点的点数，根据点数生成围绕每一个路径点的点云
                                ThetaList=[ThetaList0(:,1:end-1),PrTheta'];
                                ...
                                %% 最终，给OS的返回值
                                %TODO 怎么设置？
                                % 给OS的返回值分成两种，一种是下一次bayes推测需要用的本阶段的数据，一种是OS决策需要用的
                                Boat(OS).Infer(TS).InferHis=CalHis_TS;       %累积
                                Boat(OS).Infer(TS).CAL=[t,CAL_TS];           %累积
